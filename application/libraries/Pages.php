...
class Pages
{
    ...
    // Additional Options
    var $url_type       = 'q'; // either 'd'=directory or 'q'=query string
    ...
    var $stats_title    = 'data'; // will show the title for the page stats
    var $view_all_link  = true; // will show the view all link
    var $view_all_text  = 'Lihat Semua'; // text for $view_all_link
    ...
    public function create_links()
    {
        if ($this->total_rows > $this->rows_per_page) {
            ...
            if ($this->cur_page > 1) {
                ...
                $prev = $this->create_link('Sebelumnya', $page, true, $this->prev_class);
            } else {
                if ($this->show_prev) {
                    $prev = $this->create_link('Sebelumnya', $page, true, $this->prev_class);
                }
            ...
            if ($this->cur_page < $this->max_pages) {
                $page = $this->cur_page + 1;
                $next = $this->create_link('Selanjutnya', $page, true, $this->next_class);
            } else {
                if ($this->show_next) {
                    $prev = $this->create_link('Selanjutnya', $page, true, $this->next_class);
                }
            }
    ...
    public function create_link($text, $page_no, $is_link = true, $css_class = '')
    {
        ...
        if ($this->url_type == 'd' && $this->show_trai_sl == true && $page_no != 1) {
            ...
            if ($page_no == 1) {
                ...
                $href = $this->base_url . "?halaman=$page_no" . $pars;
            }
        }
    ...
    public function page_stats()
    {
        if ($this->total_rows == 0) {
            ...
            echo number_format($start, 0) .
                 " sampai " . number_format($end, 0) .
                 " (total " . number_format($this->total_rows, 0) .
                 " " .
                 $this->stats_title .
                 ")";
        }
